...
2. **A chain of queue jobs is pushed** (whole-project sync only — the widget's single-page sync runs inline). The controller creates a `contentiq_import_runs` row (`phase: staging`) and pushes `StageRunJob`, the first of four jobs that hand off to each other as the run advances; the CP polls `sync/status` for progress and offers Resume if a job dies mid-run. See [import-pipeline.md](import-pipeline.md#the-job-chain--phase-by-phase) for the full chain.
...
6. **Pass 3: link sweep.** Root-relative links written this run — CKEditor `<a href>`s, hero/CTA/action-button and card Hyper `Url` links — are upgraded to live Craft entry references now that every page has an entry ID. Runs immediately after pass 2, inside the same shared `ImportService::runPostPasses()` call every entry point uses for both passes. See [import-pipeline.md](import-pipeline.md#pass-3--link-sweep).
7. **Pass 4: legacy-URL redirects.** For every page written this run that carries a ContentiQ Legacy URL, a Retour static redirect (path-only, exact match, trailing-slash form) is upserted from the old path to the new entry URI — skipped silently when the two already match, and never deleting an existing redirect. Optional: Retour absent → one run-level warning. Same `runPostPasses()` call. See [import-pipeline.md](import-pipeline.md#pass-4--legacy-url-redirects).
8. **Ack** — `POST /api/v1/pages/ack` tells ContentiQ which pages were genuinely written this run. This is the only call that mutates ContentiQ's own state; a failure here is a non-fatal warning, never a failed sync — the pages are already safely in Craft.
9. **Report** — the run record is finalised (`success`/`warnings`/`errors`) and the CP renders a hierarchical sync report: per-page created/updated/skipped/failed status, image counts, and any warnings.
