...
- `redirects` — `{enabled: bool, httpCode: int}`, defaulting to `true`/`301`. Drives PASS 4 of `runPostPasses()`: `document.legacy_url` becomes a Retour static redirect to the entry's live Craft URI, for pages genuinely written this run. Requires `nystudio107/craft-retour` installed AND enabled on the target site — it's never a composer dependency of this plugin (same reasoning as Hyper/SEOmatic above); its absence is a single run-level warning, not a per-page one. Both sides of every redirect are built with Craft's own trailing-slash URL form (`/about-us/`) to match target sites' `addTrailingSlashesToUrls: true` and Retour's literal (non-normalising) match — except a file-like last segment (`/roofing.html`), which never gets a trailing slash. See `RedirectService` and [import-pipeline.md](import-pipeline.md#pass-4--legacy-url-redirects).
