...
**No drafts, ever.** Every save is `Craft::$app->getElements()->saveElement($entry, false)` directly on the canonical entry — never `getDrafts()->createDraft()`. Drafts were tried early and silently wrote images the CP couldn't see (it renders canonical, not drafts). The `false` (skip validation) is required to dodge `MatrixBlockAnchorField` uniqueness failures on nested Matrix saves, but it has a cost: URI generation is validation-time in Craft, so every write path must handle regenerating the entry's URI itself. Structure positioning triggers Craft's own `afterMoveInStructure()` hook, but that hook QUEUES its URI write rather than performing it inline, so `ImportService::positionInStructure()` itself calls `ImportService::refreshUri()` immediately after the move; collection children call the private `_refreshUri()` explicitly after every save (no structure step at all). See [docs/import-pipeline.md](docs/import-pipeline.md).
...
| The page-level pipeline: find-or-create, saves, collection children, locks, Structure positioning | [docs/import-pipeline.md](docs/import-pipeline.md) | `saveElement($entry, false)` skips URI generation — collection children need an explicit `_refreshUri()` call; pages need an explicit `ImportService::refreshUri()` call too, right after structure positioning, because `afterMoveInStructure()` queues its own URI write rather than performing it inline. `append()`/`appendToRoot()` moves an entry to the END of its siblings — position via `ImportService::positionInStructure()` (reads ContentIQ's `sort_order`), never call `append()`/`appendToRoot()`/`moveBefore()` directly |
...
| Legacy-URL redirects — ContentiQ `document.legacy_url` → Retour static redirect (`RedirectService`, pass 4) | [docs/import-pipeline.md](docs/import-pipeline.md#pass-4--legacy-url-redirects) | Retour is optional and never a composer dependency; the pass never deletes a redirect (Retour's `checkForRedirectLoop` would — keep it `false`); both slash forms of the source are written; destination is trailing-slash form; Retour treats a non-zero `associatedElementId` as a Short Link and hides it from the Redirects list — leave it 0; reading `$entry->uri` here relies on the structure-move URI refresh below — see the next hard-limit bullet |
...
- Read `$entry->uri` in the same run as a structure move without an inline `updateElementSlugAndUri($entry, true, false, false)` first — Craft queues that write
...
- Hand-build a page-result row for `runPostPasses()` — pass the `importPage()` result through (a superset is fine); the post-passes read keys like `legacyUrl` from it
- Treat a missing `contentiq_entry_syncs` row as unlocked — the safe default everywhere it's checked (Sync screen, `ImportPagesJob`, the sidebar widget) is locked
- Apply Sync-screen lock/unlock or globals-consent selections at controller-request time — `StageRunJob` defers both to job execution, so a queue worker that never runs never "leaks" an unlock against a sync that didn't happen
...
- Carry batched-sync state across a job boundary in anything but the `contentiq_import_runs` row and its `contentiq_sync_pages` rows — a job's own payload holds only its run id (plus Craft's own batch offsets). See [docs/import-pipeline.md](docs/import-pipeline.md#the-job-chain--phase-by-phase).
- Skip a job's `run.phase` check, or a batched item's own row-status check, before doing work — every job in the pipeline checks `run.phase` first (a mismatch means a stale retry or double-push, and the job does nothing) and every batched item (`ImportPagesJob`/`PostPassJob`) checks its row's own status/flag before processing it — that pair is what makes a killed batch resumable without re-processing or duplicating anything.
...
- Delete `src/jobs/SyncJob.php` before a full release has passed since 1.35.0 — it's a one-release compatibility shim that converts a pre-deploy queued `SyncJob` payload into a `StageRunJob` run instead of running a sync itself; removing it early leaves an old queued payload fatal at execution instead of converting cleanly. See [docs/import-pipeline.md](docs/import-pipeline.md#the-job-chain--phase-by-phase).
