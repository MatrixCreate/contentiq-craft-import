...
| The page-level pipeline: find-or-create, saves, collection children, locks | [docs/import-pipeline.md](docs/import-pipeline.md) | `saveElement($entry, false)` skips URI generation — collection children need an explicit `_refreshUri()` call; pages need an explicit `ImportService::refreshUri()` call too, right after structure positioning, because `afterMoveInStructure()` queues its own URI write rather than performing it inline |
